...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        ...
        if (header) {
            if (isMobile) {
                // На мобильных: скрываем если не в самом верху
                if (scrollTop > 10) {
    ...
    if (searchInput && searchBtn) {
        const handleSearch = () => {
            ...
            if (query) {
                // Determine path to catalog based on current location
                const isPagesDir = window.location.pathname.includes('/pages/');
                // If we are in pages/, we go up one level then to catalog. 
                // Wait, if we are in pages/cart/cart.html, we need ../catalog/catalog.html
                // If we are in index.html, we need pages/catalog/catalog.html
                // But window.location.pathname might not be reliable on all servers if base url varies.
                // Safest: check if we are in root or subfolder.
                // Assuming standard structure:
                // Root: index.html
                // Subfolders: pages/xxx/xxx.html
                
                let target = 'pages/catalog/catalog.html';
                if (window.location.href.includes('/pages/')) {
                    target = '../catalog/catalog.html';
                }
